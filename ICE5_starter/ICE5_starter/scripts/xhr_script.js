...
xhr.onreadystatechange = () => {
    ...
    if (xhr.readyState === 4) {
        ...
        console.log(`The state: ${xhr.status}`)

        if (xhr.status === 200) {

            console.log(`The responce: ${xhr.responseText}`)
            
            // turn into json
            let jsonResponce = JSON.parse(xhr.responseText)
            // console log to see what we have
            console.log(`the responce: ${jsonResponce[0]['url']}`)
            let photo_url_1 = jsonResponce[0]['url']
            let photo_url_2 = jsonResponce[1]['url']
            ...
            $('#photo2 ~ figcaption').text("This is the first image updated by the AJAX call.")
